...
describe('ToppingsReducer', () => {
  ...
  describe('ToppingActions.loadSuccess', () => {
    it('should populate the toppings array', () => {
      ...
      const { initialState } = fromToppings;
      const action = ToppingActions.loadSuccess({ toppings });
      const state = fromToppings.reducer(initialState, action);
  ...
  describe('ToppingActions.loadFail', () => {
    it('should return the initial state', () => {
      const { initialState } = fromToppings;
      const action = ToppingActions.loadFail({ error: null });
      const state = fromToppings.reducer(initialState, action);
  ...
  describe('ToppingActions.visualise', () => {
    it('should set an array of number ids', () => {
      const { initialState } = fromToppings;
      const action = ToppingActions.visualise({ toppingIds: [1, 5, 9] });
      const state = fromToppings.reducer(initialState, action);
